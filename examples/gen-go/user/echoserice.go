...
type EchoSerice interface {
	// Parameters:
	//  - User
	Echo(user *User) (r string, err error)
}

type EchoSericeClient struct {
...
func NewEchoSericeClientFactory(t thrift.TTransport, f thrift.TProtocolFactory) *EchoSericeClient {
	return &EchoSericeClient{Transport: t,
		ProtocolFactory: f,
...
func NewEchoSericeClientProtocol(t thrift.TTransport, iprot thrift.TProtocol, oprot thrift.TProtocol) *EchoSericeClient {
	return &EchoSericeClient{Transport: t,
		ProtocolFactory: nil,
		InputProtocol:   iprot,
...
// Parameters:
//  - User
func (p *EchoSericeClient) Echo(user *User) (r string, err error) {
	if err = p.sendEcho(user); err != nil {
		return
	}
	return p.recvEcho()
}

func (p *EchoSericeClient) sendEcho(user *User) (err error) {
...
func (p *EchoSericeClient) recvEcho() (value string, err error) {
...
type EchoSericeProcessor struct {
	processorMap map[string]thrift.TProcessorFunction
	handler      EchoSerice
}

func (p *EchoSericeProcessor) AddToProcessorMap(key string, processor thrift.TProcessorFunction) {
	p.processorMap[key] = processor
}

func (p *EchoSericeProcessor) GetProcessorFunction(key string) (processor thrift.TProcessorFunction, ok bool) {
	processor, ok = p.processorMap[key]
	return processor, ok
}

func (p *EchoSericeProcessor) ProcessorMap() map[string]thrift.TProcessorFunction {
	return p.processorMap
}

func NewEchoSericeProcessor(handler EchoSerice) *EchoSericeProcessor {

	self2 := &EchoSericeProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self2.processorMap["echo"] = &echoSericeProcessorEcho{handler: handler}
	return self2
}

func (p *EchoSericeProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
...
type echoSericeProcessorEcho struct {
	handler EchoSerice
}

func (p *echoSericeProcessorEcho) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	args := EchoArgs{}
	...
	var retval string
	var err2 error
	if retval, err2 = p.handler.Echo(args.User); err2 != nil {
...
var EchoArgs_User_DEFAULT *User

func (p *EchoArgs) GetUser() *User {
	if !p.IsSetUser() {
		return EchoArgs_User_DEFAULT
	}
	return p.User
}
func (p *EchoArgs) IsSetUser() bool {
	return p.User != nil
}
